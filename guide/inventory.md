...
| `externalChangesOnly` | `bool` | If true, only return variants with inventory changes that have happened outside of sales on this channel. | `false` |
...
> [!NOTE]
> `externalChangesOnly` should only be used if your channel is confident it can keep its own
> internal inventory account for orders that happen directly on your channel.
